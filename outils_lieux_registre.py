"""Almaval - moteur de l'occupation des bureaux : le registre des attributions.

Ce module porte le registre Attributions du classeur des lieux et le
passage quotidien. Il s'appelait outils_lieux_transitoire jusqu'au
23.09.2026, du nom du mode decide au Conseil de direction du 14.09.2026
et presente alors comme provisoire, en attendant le gestionnaire RH 2.0.
Alberto a tranche le 23.09.2026 : ce mode devient LA REGLE, et le nom du
module a suivi pour ne pas mentir sur ce qu'il fait.

L'onglet Attributions est OUVERT a Clement Berger, qui y ecrit et corrige
a la main les attributions officielles, la grille Propositions restant un
brouillon qui ne remonte plus toute seule dans le registre.

lieux_charte_attributions, plus bas, est la SEULE fonction qui ecrit cet
onglet. La charte generale, lieux_poser_la_charte, ne le touche plus du
tout depuis le 23.09.2026 : les deux se contredisaient, celle-la
refermant les six colonnes que celle-ci ouvre, et le droit d'ecriture de
Clement disparaissait a chaque passage. Une protection, un ecrivain.
...
Le nom de ce module compte pour l'ordre de chargement : outils_lieux_zz_passage
et outils_zzz_lieux_quota enveloppent son passage quotidien et doivent donc
etre charges APRES lui, ce que l'ordre alphabetique de bootstrap assure.
"""
...
# Les onze colonnes d'origine du registre, plus les deux ajoutees le
# 14.09.2026, « Fin selon registre RH » et « Origine ».
COLONNES = [
...
def lieux_consolider_attributions(sujet: str = ""):
    ...
    Regle de la maison (14.09.2026, confirmee le 23.09.2026) : Clement
    Berger ecrit dans l'onglet
...
def lieux_charte_attributions(sujet: str = ""):
    ...
    _journaliser([[_maintenant(), "Charte", "Attributions, registre ouvert", ONGLET_ATTRIBUTIONS, "",
...
@mcp.tool()
@tolerant
def lieux_poser_toute_la_charte(sujet: str = ""):
    """La charte du classeur entier, puis celle du registre des attributions.

    S'appelait lieux_poser_la_charte_transitoire jusqu'au 23.09.2026.
    Les deux fonctions appelees ne se recouvrent plus : depuis cette date
    lieux_poser_la_charte ne touche plus l'onglet Attributions, qui
    appartient a lieux_charte_attributions seule.
    """
...
def lieux_construire_attributions(sujet: str = ""):
    ...
    de fin du registre RH, statuts, colonnes du registre realignees,
    l'ancien passage n'ecrivant que onze colonnes).
    """
...
def lieux_cycle(sujet: str = ""):
    """Le passage complet du registre des attributions, sans confirmation.
...
for _nom, _fn in (("lieux_construire_attributions", lieux_construire_attributions), ("lieux_cycle", lieux_cycle)):
    try:
        ...
        print("[lieux registre] " + _nom + " non remplacé : " + type(exc).__name__ + " " + str(exc)[:200], flush=True)
print("[lieux registre] outils remplacés : " + (", ".join(_remplaces) or "aucun"), flush=True)
